...
export const UserProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      const { data } = await httpService.post("http://127.0.0.1:8000/api/v1/auth/login/", {
        email,
        password,
      });
      // httpService.setJwt(data.access);
      console.log(data);
      setAuthTokens({ access: data.access_token, refresh: data.refresh });
      setUserInfo({
  ...
  const register = async (first_name,last_name,username, email, password) => {
    try {
      const { data } = await httpService.post("http://127.0.0.1:8000/api/v1/auth/register/", {
        first_name,
        last_name,
        email,
        password,
